...
function ModoEnvio() {
  ...
  const [subEstados, setSubEstados] = useState({}); // id -> {estado, total_enviados, total_errores, total_validos}
  ...
  // Logs del envío normal (solo aplica si NO se dividió)
  useEffect(() => {
  ...
  // Progreso de los sub-envíos (solo aplica si SÍ se dividió)
  useEffect(() => {
  ...
  function nueva() {
    setNombre(""); setPlantilla(""); setImagenUrl(""); setEnvioId(null); setDepu(null); setLogs([]); setEstado(null); setEnviando(false);
  ...
  // Estado agregado de los sub-envíos, para la vista dividida
